...
export async function generateStaticParams() {
    return posts.map((post) => ({
        id: post.slug.split('/').pop(),
    }));
}

export default function BlogPost({ params }: { params: { id: string } }) {
    const post = posts.find((p) => p.slug.split('/').pop() === params.id);
    ...
    return (
        <article className="max-w-3xl mx-auto">
            ...
            <div className="prose prose-invert prose-code:text-terminal-green prose-pre:bg-black/50 prose-headings:font-bold prose-headings:text-white text-gray-300 max-w-none">
